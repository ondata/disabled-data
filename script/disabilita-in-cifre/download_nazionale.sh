...
cat "$folder"/tmp/anni-amministrativa.jsonl | while read -r line; do
  ...
  cat "$folder"/tmp/"$anno"-amministrativa.jsonl | grep 'nazionale' | while read -r line; do
  #cat "$folder"/tmp/2019-amministrativa.jsonl | grep 'nazionale' | while read -r line; do
    URL=$(echo "$line" | jq -r '.href')
