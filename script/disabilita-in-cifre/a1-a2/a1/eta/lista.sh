...
while read line; do
  ...
  while read foglio; do
    ...
    echo '{"file":"'"$name"'","tavola":"'"$tavola"'","pagina":"'"$pagina"'"}' >>"$folder"/processing/out.jsonl
...
mlr --j2c put '$regione=sub($tavola,"^(.+?)( - )(.+?) ([(].+)$","\3");$regione=sub($regione,"Regione *","");$tavola=sub($tavola,"^(.+?)( - )(.+?) ([(].+)$","\1");$pagina=sub($pagina,"^.+= ","");$pagina=sub($pagina,"\.","")' "$folder"/processing/out.jsonl >"$folder"/processing/out.csv
...
mv "$folder"/processing/out.csv "$folder"/processing/a2-eta.csv
