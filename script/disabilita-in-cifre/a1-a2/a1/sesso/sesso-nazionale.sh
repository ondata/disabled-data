...
# i primi tre file
find "$folder"/nazionale -maxdepth 1 -iname '*.xls' >"$folder"/tmp
...
while read line; do
  ...
  while read foglio; do
    ...
    echo "$tavola"
    echo "$pagina"
    qsv excel -s "$foglio" "$line" | grep -v ',,Gravità de' | mlr --csv -N clean-whitespace then remove-empty-columns then skip-trivial-records then put 'if(is_empty($1)){$1="anno"}' then filter -S '$1=~"^(anno|[0-9]+)"' | mlr --c2j put '$file="'"$name"'";$tavola="'"$tavola"'";$pagina="'"$pagina"'"' >>"$folder"/processing/out.jsonl
...
mv "$folder"/processing/out.csv "$folder"/../../../../../data/disabilita-in-cifre/processing/a1-a2/a1-sesso-nazionale.csv
